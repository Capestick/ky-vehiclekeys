...
RegisterNetEvent('ky-vehiclekeys:server:GiveVehicleKeys', function(receiver, plate)
    local giver = source
...
RegisterNetEvent('ky-vehiclekeys:server:AcquireVehicleKeys', function(plate)
    local src = source
    GiveKeys(src, plate)
end)

RegisterNetEvent('ky-vehiclekeys:server:breakLockpick', function(itemName)
...
QBCore.Functions.CreateCallback('ky-vehiclekeys:server:GetVehicleKeys', function(source, cb)
...
function GiveKeys(id, plate)
    ...
    TriggerClientEvent('ky-vehiclekeys:client:AddKeys', id, plate)
end
...
function RemoveKeys(id, plate)
    ...
    TriggerClientEvent('ky-vehiclekeys:client:RemoveKeys', id, plate)
end
...
QBCore.Commands.Add("engine", Lang:t("addcom.engine"), {}, false, function(source)
	TriggerClientEvent('ky-vehiclekeys:client:ToggleEngine', source)
end)
...
QBCore.Commands.Add("givekeys", Lang:t("addcom.givekeys"), {{name = Lang:t("addcom.givekeys_id"), help = Lang:t("addcom.givekeys_id_help")}}, false, function(source, args)
	local src = source
    TriggerClientEvent('ky-vehiclekeys:client:GiveKeys', src, tonumber(args[1]))
end)
