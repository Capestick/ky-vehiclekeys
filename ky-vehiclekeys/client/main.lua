            ...
            if LocalPlayer.state.isLoggedIn then
                ...
                if entering ~= 0 and not isBlacklistedVehicle(entering) then
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                        if IsEntityDead(driver) then
                            if not isTakingKeys then
                                isTakingKeys = true
                                TriggerServerEvent('ky-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                                QBCore.Functions.Progressbar("steal_keys", Lang:t("progress.takekeys"), 2500, false, false, {
                                    ...
                                    TriggerServerEvent('ky-vehiclekeys:server:AcquireVehicleKeys', plate)
                                    isTakingKeys = false
                                end, function()
                                    isTakingKeys = false
                                end)
                        ...
                        elseif Config.LockNPCDrivingCars then
                            TriggerServerEvent('ky-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                        else
                            TriggerServerEvent('ky-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            SetVehicleEngineOn(entering, false, true, true)
                    ...
                    elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                        QBCore.Functions.TriggerCallback('ky-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
                ...
                local isHotwiring = false -- Track hotwiring status
                local uiDisplayed = false -- Track whether the UI is currently displayed
                
                -- Function to show UI and play alarm sound
                local function ShowUI()
                    local message = "Press E to Lockpick"
                    exports['lab-TextUI']:Show(message) -- Show text with 'lab-TextUI'
                    SendNUIMessage({ action = 'playSound' }) -- Play the alarm sound
                    uiDisplayed = true
                end
                
                -- Function to hide UI
                local function HideUI()
                    exports['lab-TextUI']:Hide() -- Hide text with 'lab-TextUI'
                    uiDisplayed = false
                end
...
RegisterNetEvent('ky-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped)
...
RegisterNetEvent('ky-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)
RegisterNetEvent('ky-vehiclekeys:client:ToggleEngine', function()
...
AddEventHandler('ky-vehiclekeys:client:setLastPickedVehicle', function(vehicle)
    lastPickedVehicle = vehicle
end)


RegisterNetEvent('ky-vehiclekeys:client:GiveKeys', function()
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    TriggerServerEvent('ky-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
                    QBCore.Functions.Notify(Lang:t("notify.vlock"), "primary")
                else
                    TriggerServerEvent('ky-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('ky-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
        QBCore.Functions.Notify(Lang:t("notify.nonear"),'error')
    end
end
function GetKeys()
    QBCore.Functions.TriggerCallback('ky-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
            ...
            for _, vehicle in pairs(v.vehicles) do
                if string.upper(vehicle) == string.upper(vehName) then
                    if not HasKeys(vehPlate) then
                        TriggerServerEvent("ky-vehiclekeys:server:AcquireVehicleKeys", vehPlate)
                    end
                    return true
                end
            end
...
function LockpickDoor(isAdvanced)
    ...
    if boostingInfo ~= nil and ((not boostingInfo.groupIdentifiers and boostingInfo.cid ~= PlayerData.citizenid) or (boostingInfo.groupIdentifiers and not boostingInfo.groupIdentifiers[PlayerData.citizenid])) then
...
function LockpickFinishCallback(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('ky-vehiclekeys:server:AcquireVehicleKeys', QBCore.Functions.GetPlate(vehicle))
        else
            QBCore.Functions.Notify(Lang:t("notify.vlockpick"), 'success')
            TriggerServerEvent('ky-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 1)
        end
    ...
    if usingAdvanced then
        if chance <= Config.RemoveLockpickAdvanced then
            TriggerServerEvent("ky-vehiclekeys:server:breakLockpick", "advancedlockpick")
        end
    else
        if chance <= Config.RemoveLockpickNormal then
            TriggerServerEvent("ky-vehiclekeys:server:breakLockpick", "lockpick")
        end
    end
end
